...
export class HFCClientManager{
    ...
    /**
     * Returns channel details for the org
     */
    public async getChannelDetails(org: string,userId:string):Promise<any>{
        var client : HFCClient = this.clientMap[org]
        var trxnResult = await client.getChannels(org+"_"+userId)
        _LOGGER.info(`Transaction result ${trxnResult}`)
        return new Promise<any>((resolve)=>resolve(trxnResult))
    }
    /**
     * Returns installed chanin codes in a organization
     */
    public async getInstalledChaincode(org: string,userId:string):Promise<any>{
        var client : HFCClient = this.clientMap[org]
        var trxnResult = await client.getInstalledChaincode(org+"_"+userId)
        _LOGGER.info(`Transaction result ${trxnResult}`)
        return new Promise<any>((resolve)=>resolve(trxnResult))
    }
    /**
     * Returns instantiated  chain codes in a channel
     */
    public async getInstantiatedChaincodes(channel: string,org: string,userId:string):Promise<any>{
        var client : HFCClient = this.clientMap[org]
        var trxnResult = await client.getInstantiatedChaincode(channel,org+"_"+userId)
        _LOGGER.info(`Transaction result ${trxnResult}`)
        return new Promise<any>((resolve)=>resolve(trxnResult))
    }
    /**
     * Returns block details of a block
     */
    public async getBlockDetails(blockNumber:number, channel: string,org: string,userId:string):Promise<any>{
        var client : HFCClient = this.clientMap[org]
        var trxnResult = await client.getBlockDetails(channel,blockNumber,org+"_"+userId)
        _LOGGER.info(`Transaction result ${trxnResult}`)
        return new Promise<any>((resolve)=>resolve(trxnResult))
    }
    /**
     * Returns transaction details of an input transaction id
     */
    public async getTrxnDetails(trxnId:string, channel: string,org: string,userId:string):Promise<any>{
        var client : HFCClient = this.clientMap[org]
        var trxnResult = await client.getTrxnDetails(channel,trxnId,org+"_"+userId)
        _LOGGER.info(`Transaction result ${trxnResult}`)
        return new Promise<any>((resolve)=>resolve(trxnResult))
    }
}
...
/**
 * Block event callback functon 
 */
export interface BlockEventCallBack {
    (peerId: string, block: any): void;
}
/**
 * Disconnect event callback function
 */
export interface EventHubDisconnectCallBack {
...
export class HFCClient {
    ...
    private lastBlockEventRegNum: number = 0
    ...
    /**
     * Registers for the block event 
     */
    public async registerForBlockEvent(blockRecvCB: BlockEventCallBack, disconnectCB: EventHubDisconnectCallBack): Promise<boolean> {
        let isSuccess = false
        try {
            var isAdminLoaded = await this.getOrgAdmin()
            if (isAdminLoaded) {

                this.client.setUserContext(this.adminUser);
                this.eventHub = this.client.newEventHub()
                this.setupEventHub()
                this.lastBlockEventRegNum = this.eventHub.registerBlockEvent(
                    (block) => {
                        ...
                        this.logger.info(`Block created ${block.header.number} on channel ${channel_id}`)
                        blockRecvCB("", block)

                    },
                    (err) => {
                        this.logger.warn('Disconnected from event hub');
                        this.eventHub.unregisterBlockEvent(this.lastBlockEventRegNum)
                        disconnectCB("", err)
                    }
                );
                this.logger.info("Before connect")
                this.eventHub.connect()
                this.logger.info("After connect")
            ...
            this.logger.error(`Connet to event hub`, exp)
            isSuccess = false
        }
        return new Promise<boolean>((resolve) => { resolve(isSuccess) });
    }
    ...
    /**
     * Query instantiated chaincodes in a channel
     * @param channelId string 
     * @param userId string userid
     */
    public async getInstantiatedChaincode(channelId: string, userId: string): Promise<any> {
        let queryOutput = {}
        let channel: any = null
        try {
            if (this.channelMap[channelId] != null) {
            ...
            if (appUser != null) {
                this.client.setUserContext(appUser)
                var targets = this.setupPeers(null)
                queryOutput = channel.queryInstantiatedChaincodes(targets[0])

            } else {
                this.logger.error(`Application user ${userId} enrollment failure`)
                queryOutput = "Unable to get the user credentials"
            }
        } catch (exp) {
            this.logger.error(`getInstantiatedChaincode for userId ${userId} ${channelId}`, exp)
            queryOutput = "TRXN_FAILED"
        }
        return new Promise<any>((resolve) => { resolve(queryOutput) })
    }
    ...
    * @param userId string userid
    */
    public async getBlockDetails(channelId: string, blockNum: number, userId: string): Promise<any> {
        let queryOutput = {}
        let channel: any = null
        try {
            if (this.channelMap[channelId] != null) {
            ...
            if (appUser != null) {
                this.client.setUserContext(appUser)
                var targets = this.setupPeers(null)
                queryOutput = channel.queryBlock(blockNum, targets[0])
            ...
            this.logger.error(`getBlockDetails for userId ${userId} ${channelId}`, exp)
            queryOutput = "TRXN_FAILED"
        }
        return new Promise<any>((resolve) => { resolve(queryOutput) })
    }
    ...
    * @param userId string userid
    */
    public async getTrxnDetails(channelId: string, trxnId: string, userId: string): Promise<any> {
        let queryOutput = {}
        let channel: any = null
        try {
            if (this.channelMap[channelId] != null) {
            ...
            if (appUser != null) {
                this.client.setUserContext(appUser)
                var targets = this.setupPeers(null)
                queryOutput = channel.queryTransaction(trxnId, targets[0])
            ...
            this.logger.error(`getTrxnDetails for userId ${userId} ${channelId}`, exp)
            queryOutput = "TRXN_FAILED"
        }
        return new Promise<any>((resolve) => { resolve(queryOutput) })
    }
    ...
    public async getChannels(userId: string): Promise<any> {
        ...
        try {
            ...
            if (appUser != null) {
                this.client.setUserContext(appUser)
                var targets = this.setupPeers(null)
                queryOutput = await this.client.queryChannels(targets[0])
                this.logger.info(JSON.stringify(queryOutput))
            } else {
                this.logger.error(`Admin enrollment failure`)
                queryOutput = "Admin enrollment dailure"
            }
        } catch (exp) {
            this.logger.error(`Query of channels failed`, exp)
            queryOutput = "TRXN_FAILED"
        }
        return new Promise<any>((resolve) => { resolve(queryOutput) })
    }
    ...
    * @param userId string userid
    */
    public async getInstalledChaincode(userId: string): Promise<any> {
        let queryOutput: any = {}
        let channel: any = null
        try {
            var appUser = await this.getUser(userId)
            if (appUser != null) {
                ...
                var targets = this.setupPeers(null)
                queryOutput = await this.client.queryInstalledChaincodes(targets[0])
                this.logger.info(JSON.stringify(queryOutput))
            ...
            this.logger.error(`Query of installedChaincode failed`, exp)
            queryOutput = "TRXN_FAILED"
        }
        return new Promise<any>((resolve) => { resolve(queryOutput) })
    }
 ...
 */
    ...
    * Add the peers to the peer map. Will be used during channel creation
    */
    private setupEventHub() {

        for (let key in this.organizationConfig) {
            if (key.indexOf('peer') === 0) {
                let data = fs.readFileSync(path.join(__dirname, this.organizationConfig[key]['tls_cacerts']));
                this.logger.info(`Hooking to Peer ${this.organizationConfig[key].events} for block events`)
                this.eventHub.setPeerAddr(
                    this.organizationConfig[key].events, {
                        pem: Buffer.from(data).toString(),
                        'ssl-target-name-override': this.organizationConfig[key]['server-hostname'],
                        'request-timeout': 5000000
                    }
